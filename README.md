...
Фильтр может быть использован при решении различных задач управления
...
При "медленно меняющихся" входных значениях выходная последовательность
...
Не простой задачей является вычисление оптимального значения величины r[i] для
...
Для текущего зачтения скорости и ускорения вычисляется "минимальный тормозной
путь" - величина перемещения при оптимальном торможении до нулевой скорости
и нулевого ускорения с ограничением скорости, ускорения и рывка.

Если "пора/нужно тормозить", то выбирается рывок для данной фазы оптимального
торможения.
...
- тормозной путь после такта с максимальным отрицательным рывком.
Если заданная точка находится "в трубке", то рывок вычисляется пропорционально
промахам. Если трубка "не дотягивает" или "перепрыгивает" заданное значение,
то берется максимальный/минимальный рывок.
...
ускорения, то может использоваться следующий код:
...
ускорения и рывку произвести перемещение за время не более одного такта dt
в заданную точку.
...
4. Реверс
Направление движения (знак скорости) не соответствует разнице
...
превышает разницу заданного и текущего положения.
...
Примечание:
  На начальной фазе программного торможения из-за ограничения
  рывка знаки могут соответствовать режму "Разгон", пока ускорение
  не помянет знак на соответствующий снижению скорости.
...
тормозной путь (время торможения) для положительного значения
...
торможение с постоянным рывком R до полной остановки (v = a = 0):
```
2 * V * R = A^2
T = A / R
```
...
то торможение может состоять из 2-х или 3-х фаз.
...
Из-за сравнительно небольшой начальной скорости за время тормозного
пути ускорение торможения не успевает достигнуть максимально допустимого
значения Ad.
...
Тормозной путь второй фазы - площадь трапеции с основаниями
...
- 2 или 3 фазы торможения после движения с постоянной скоростью (см. выше).
За счет "выбега" (изменения знака скорости) торможение происходит "долго".

Ответ на вопрос "почему?": Во время движения с большой скоростью произошло
крутое изменение заданного значения. Во время действия большого ускорения
(ускорения разворота) скорость упала. Если скорость |V| < A^2/(2 * R),
то не возможно с ограниченным рывком обнулить одновременно скорость
и ускорение.

Если модуль скорости меньше, чем v1 = a^2 / (2 * R), то торможение будет с
"выбегом" с большим числом фаз.

Если модуль скорости больше, чем v2 = (Ad^2 - a[i]^2 / 2) / R, то торможение
