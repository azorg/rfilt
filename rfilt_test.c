...
int main()
{
  ...
  FILE *f = fopen("data.txt", "wb"); // файл данных
  ...
  rfilt_init(
    ...
    80.,     // максимальное ускорение торможения
    200.,    // максимальный рывок
    0.01,    // коэффициент запаса фильтра [0..0.1]
    0.,      // начальное положение
    -100.,   // ограничение снизу
    100.);   // ограничение сверху
