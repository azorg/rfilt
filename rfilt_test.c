...
int main()
{
  ...
  rfilt_init(
    ...
    30.,     // максимальное ускорение разгона
    30.,     // максимальное ускорение торможения
    45.,     // максимальный рывок
  ...
  for (i = 0; i < n; i++) d[i] = 0.;
  for (i = 20; i < 820; i++) d[i] = 50.;

#if 1
  for (i = 820; i < 1120; i++) d[i] = -30.;

  for (i = 1400; i < (1400+1000); i++) // 2 периода
    d[i] = 10. * sin((i - 1400.) * 2 * M_PI / 500.); // период 5 сек
