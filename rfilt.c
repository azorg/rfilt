...
void rfilt_init(
  ...
  double beta,       // коэфицент бета фильтра
...
{
  double A, ds, r, v, a, xs;
  double a2 = self->a * self->a; // квадрат текущего ускорения
  double s;
  ...
  RFILT_DBG("work: x=%f v=%f a=%f r=%f ds=%f",
             self->x, self->v, self->a, r, ds);
...
//  R *= 0.90;
  ...
  // тормозной путь с максимальным рывком
  ...
  { // x<=xs<y - управляемое движение в нужном направлении
    ...
    { // умное пропорциональне управление! FIXME
      ...
      double z = ds - s; // невязка, если начать тормозить сразу на максималках (z>0)
        
      if (ds == 0.)
      { // редкий частный (вырожденный) случай s=ds=0 
        r = 0.; // никуда не разгоняемся
      }  
      else if (0 && z < ds * self->B) // FIXME: magic
      { // при завершении большей части разгона
        // заблаговременно начинаем тормозить c расчётным рывком
        double Rd = R * (1 - self->B); // FIXME        
        RFILT_DBG("z=ds-s=%f ds=%f s=%f Rd=%f", z, s, ds, Rd);
...
#if 1 // ограничить рывок в связи с ограничением ускорения по модулю
  if (a + r >  A) r =  A - a;
  if (a + r < -A) r = -A - a;
  // возмозна ситуация смены знака скорости при этом возникает
  // большой рывок с Ad на Au (или наоборот)
  // r = RFILT_LIMIT_ABS(r, R); // FIXME этот код НЕ НУЖЕН
#endif

#if 1 // FIXME этот код пока еще плох!
  // проверка превышениия максимальной скорости
  if (v >=  self->V && r > 0.) r = 0.;
  if (v <= -self->V && r < 0.) r = 0.;
  ...
  if ((a > 0. && v >  vcrit) || // скорость растёт и достигнет V
      (a < 0. && v < -vcrit))   // скорость падает и достигнет -V
  { // достигнута критическая скорость, требуется немедленное сижение ускорения
    // во избежание превышения максимальной скорости
    // задаем максимальный рывок с целью снижения ускорения по модулю
#ifdef RFILT_DEBUG
    double vapprox;
    if (a > 0.) vapprox = v + a2 / (2. * R);
    else        vapprox = v - a2 / (2. * R);
#endif
    r = RFILT_LIMIT_ABS(-self->a, R);

    RFILT_DBG("limit Vmax: v=%f a=%f r=%f vcrit=%f vapprox=%f",
              v, a, r, vcrit, vapprox);
  }
#endif
